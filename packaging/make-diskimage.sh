...
if [ -d ${SRC_FOLDER}/.background ]; then
  rm -rf ${SRC_FOLDER}/.background
fi

mkdir ${SRC_FOLDER}/.background
cp dmgBackground.png ${SRC_FOLDER}/.background/
...
sudo chmod -Rf go-w "${MOUNT_DIR}" || true
